...
echo $bin --actions simu --label data-mimic --version v5 --yscale-values-list 5:10:15 --xscale-values-list 1.5 --xshift-values-list 2.4 --simu-extra-args=\"--carry-cap-values 1250 --init-population-values 32 --time-to-sampling-values 20 --n-seqs-range 60 95 --n-trials 120\" $dlargs $dlxtra $common --n-sub-procs 30
# exit 0

datadir=/fh/fast/matsen_e/data/taraki-gctree-2021-10/iqtree-processed-data/$iqvsn  # actual data
dlabel=data-iqtree-trained-iqtree-data; dvsn=v6
...
echo $bin --actions data:check-dl:replay-plot-ckdl $common --label $dlabel --version $dvsn --data-samples-list $dsl $xargs $dlargs $dlxtra
