...
pranges="--xscale-range-list 0.01,2 --xshift-range-list=-0.5,3 --yscale-range-list 0.25,7.5 --time-to-sampling-range-list 17,25 --n-seqs-range-list 40,90"
echo $bin $common --label for-data --version v2 --carry-cap-range-list 750,1500 $pranges --n-replicates 1 --n-trials-list 5000:10000:15000 --simu-extra-args=\"--n-max-procs 20 --initial-birth-rate-range 0.1 0.5 --init-population 128\" --learning-rate-list 0.001 --epochs-list 500 --batch-size-list 32 --simu-bundle-size-list 1:2:3 --dl-bundle-size-list 1:2:3 --zip-vars simu-bundle-size:dl-bundle-size:n-trials
exit 0
...
# maybe this is right (but can't read it now)?
dld=/fh/fast/matsen_e/dralph/partis/gcdyn/retrain/v1/n-trials-50000/epochs-500/batch-size-32/learning-rate-0.001/dl-infer
dvsn=v1
echo ./projects/cf-gcdyn.py --actions data --n-max-procs 5 --n-sub-procs 100 --base-outdir /fh/fast/matsen_e/dralph/partis/gcdyn --label test-data --version $dvsn --dry --dl-model-dir $dld --dl-bundle-size-list 1
