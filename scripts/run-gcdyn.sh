...
dlxtra="--dl-model-label-str for-data/v6/n-trials-50000 --dl-model-vars model-type"
...
echo $bin --actions simu --label data-mimic --version v7-yscale --yscale-values-list 5:7.5:10:15:20:30 $dlargs $dlxtra $common $dmargs
...
echo $bin --actions simu --label sig-ceil --version test-v0 $dlargs $dlxtra $common --simu-extra-args=\"--xscale-values 1.79 --xshift-values 0.79 --yscale-values 15.54 --yshift-values 0.54 --death-values 0.41 --carry-cap-values 1171 --init-population-values 64 --time-to-sampling-values 33 --n-seqs-range 120 121 --n-trials 120\" --n-sub-procs 30
echo $bin --actions simu --label sig-ceil --version test-v1 $dlargs $dlxtra $common --simu-extra-args=\"--xscale-values 1.5 --xshift-values -0.1 --yscale-values 2.5 --yshift-values 0.6 --death-values 0.5 --carry-cap-values 1000 --init-population-values 2 --time-to-sampling-values 30 --n-seqs-range 120 121 --n-trials 120\" --n-sub-procs 30
...
# TODO
# --death-values 
pargs="--params-to-predict xscale xshift yscale" # yshift
echo $bin --actions data:check-dl:replay-plot-ckdl:dl-infer $common --label $dlabel --version $dvsn --data-samples-list $dsl $xargs $dlargs $dlxtra --check-dl-input $pargs
