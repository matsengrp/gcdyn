...
def generate_sequences_and_tree(
    birth_resp, death_resp, mutation_rate, mutator, seed=0,
):
    ...
    if len(tree) < n_to_sample:
        print('  %s --n-seqs set to %d but tree has only %d tips, so just sampling all of them' % (color('yellow', 'warning'), n_to_sample, len(tree)))
...
def get_responses(xscale, xshift):
    ...
    bresp = get_birth(args.yscale)  # have to set it once so we can get the value at x=0 (at least i don't know how else to do it)
    yscl = args.yscale * args.initial_birth_rate / bresp.λ_phenotype(0)
    print('    setting yscale so resp(x=0) equals --inital-birth-rate: <yscale> * <--initial-birth-rate>/<birth rate at x=0> = %.2f * %.2f / %.4f = %.2f' % (args.yscale, args.initial_birth_rate, bresp.λ_phenotype(0), yscl))
...
parser.add_argument('--xscale-list', default=[2], type=float, nargs='+', help='parameters (see also {x,y}{scale,shift}) for birth (constant, sigmoid and soft-relu) response. If more than one is specified, one is chosen at random for each tree.')
parser.add_argument('--xshift-list', default=[-2.5], type=float, nargs='+')
parser.add_argument('--yscale', default=1, type=float, help='probably don\'t want to set this, it\'s rescaled automatically below')
parser.add_argument('--yshift', default=0, type=float, help='atm this shouldn\'t (need to, at least) be changed')
parser.add_argument('--initial-birth-rate', default=5., type=float, help='initial/default/average growth rate (i.e. when affinity/x=0). Used to set --yscale.')
parser.add_argument('--mutability-multiplier', default=0.68, type=float)
parser.add_argument('--n-sub-procs', type=int)
parser.add_argument('--itrial-start', type=int, help='if running sub procs (--n-sub-procs) set this so each sub procs trial index starts at the proper value')
...
if args.n_sub_procs is not None:  # this stuff is all copied from partis utils.py, gd it this would be like three lines if i could import that
    procs = []
        ...
        for iproc in range(len(procs)):
            ...
            if procs[iproc].poll() is not None:  # it just finished
                ...
                if procs[iproc].returncode == 0:
                    ...
                    raise Exception('process %d failed with exit code %d' % (iproc, procs[iproc].returncode))
        sys.stdout.flush()
...
if args.birth_response == 'sigmoid':
    assert all(x > 0 for x in args.xscale_list) and args.yscale > 0  # necessary so that the phenotype increases with phenotype (e.g. affinity)
...
for itrial in range(1, args.n_trials + 1):
    ...
    all_trees.append({'tree' : renamed_tree, 'birth-response' : birth_resp, 'death-response' : death_resp})

if args.dont_run_new_simu:
    print('    --dont-run-new-simu: missing %d trees, but ignoring and just merging the ones we have' % n_missing)
if n_missing > 0:
    print('    %s missing %d / %d trees (it\'s generally expected that some will fail, so this is probably ok if it\'s not too many)' % (color('yellow', 'warning'), n_missing, args.n_trials))
