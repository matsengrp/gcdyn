...
def get_kth_coordinates_of(a, k):
    """Say that we number the entries of the matrix from left to right then top
    to bottom. In what cell does the input integer k appear?
...
def sample_coordinates_from(rng, a):
    """If we probability normalize all of the entries of a 2d array and sample
    from the corresponding categorical distribution, get the coordinates of the
    sampled entry."""
...
class EventSampler:
    ...
    with exponentially-distributed time to events.

    In this implementation, rates are laid out in a matrix with events
    on the rows and things on the columns.
    """
